...
const Pagination = ({ handlePage, page, totalHotel = 55 }) => {
    const handleNext = () => {
        handlePage(
            Math.round(totalHotel / ITEM_PAGE_PER) >= page ? page + 1 : page
        );
    };
                    ...
                    <div className="hidden sm:flex sm:flex-1 sm:items-start sm:justify-between sm:flex-col sm:space-y-3">
                        <div className="mr-2">
                            <p className="text-sm text-gray-700">
                                ...
                                to{" "}
                                <span className="font-medium">
                                    {page * ITEM_PAGE_PER > totalHotel
                                        ? totalHotel
                                        : page * ITEM_PAGE_PER}
                                </span>{" "}
                                of <span className="font-medium">{totalHotel}</span> results
                            </p>
                        </div>
                        <div>
                            <nav
                            ...
                            >
                                ...
                                {Array.from({
                                    length: Math.ceil(totalHotel / ITEM_PAGE_PER),
                                }).map((item, index) => (
                                    <p
                                        key={index}
                                        onClick={() => {
