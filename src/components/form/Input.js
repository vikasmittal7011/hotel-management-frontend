const Input = ({ name, placeholder, id, onClick, type = "text", required = true, value, mt = "8", text = "xl" }) => {
    ...
    return (
        <div className={`mt-${mt} `}>
            <label htmlFor={name} className={`block text-${text} font-medium leading-6 text-gray-900 pb-1`}>{name}</label>
            <div className="mt-2">
